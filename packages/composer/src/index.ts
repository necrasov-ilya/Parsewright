...
export function composeDeterministic(input: DeterministicComposeInput): DeterministicComposeResult {
  ...
  if (input.strategy.kind === "collection") {
    ...
    if (count === 0) {
      return { answersGoal: false, answer: "No items were found on the page.", title: "No items found", issues: ["empty_collection"] };
    }
    ...
    return { answersGoal: true, answer, title: `${count} items found`, issues };
  }
  ...
  return { answersGoal: missing.length === 0, answer: summary, title: input.goal.slice(0, 40), issues };
}
